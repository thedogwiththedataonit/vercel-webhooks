...
export async function validateProjectGitConnection(
  ...
  try {
    ...
    // Fetch project details using getProjects with search
    // Note: getProjects returns an array, so we search for the specific project
    ...
    // Get the first (and should be only) project from the results
    ...
    // The 'link' property contains Git connection details
    const hasGitConnection = Boolean(project.link?.type);
    const gitProvider = project.link?.type; // 'github', 'gitlab', 'bitbucket', etc.
    
    // Get repository name - different providers use different property names
    let repository: string | undefined;
    if (project.link) {
      const link = project.link as any; // Type assertion to access provider-specific fields
    ...
    console.log('✅ Git connection validated:', {
      projectId,
      projectName: projectName || project.name,
      gitProvider,
      repository,
    ...
    console.error('❌ Failed to validate Git connection:', {
      projectId,
      projectName,
      error: errorMessage,
      timestamp: new Date().toISOString(),
...
export async function handleProjectCreatedEvent(
  ...
  console.log('🔍 Validating Git connection for new project:', {
